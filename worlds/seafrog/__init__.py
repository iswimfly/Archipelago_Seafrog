...
from .Items import base_id, item_table, group_table, BRCType, SeafrogType
from .Locations import location_table, event_table
from .Regions import region_exits
from .Rules import rules
from .Options import SeafrogOptions


class SeafrogWeb(WebWorld):
    theme = "ocean"
    tutorials = [Tutorial(
        "Multiworld Setup Guide",
        "A guide to setting up Seafrog randomizer and connecting to an Archipelago Multiworld",
        "English",
        "setup_en.md",
        "setup/en",
        ["iswimfly"]
...
class SeafrogWorld(World):
    """Master Seafrog's rocket-powered wrench as you slide, grind and trick your way through challenging skatepark-like stages at sea! Upgrade your wrench, take on crusty creatures and crazy challenges, all while seeking a way to escape a mysterious, ever-growing sinkhole in the ocean..."""
    game = "Seafrog"
    web = SeafrogWeb()
    ...
    options_dataclass = SeafrogOptions
    options: SeafrogOptions

    def __init__(self, multiworld: MultiWorld, player: int):
        super(SeafrogWorld, self).__init__(multiworld, player)
    ...
    def create_item(self, name: str) -> "SeafrogItem":
        item_id: int = self.item_name_to_id[name]
        item_type: BRCType = self.item_name_to_type[name]
        classification = self.get_item_classification(item_type)

        return SeafrogItem(name, classification, item_id, self.player)

    def create_event(self, event: str) -> "SeafrogItem":
        return SeafrogItem(event, ItemClassification.progression_skip_balancing, None, self.player)
    ...
    def create_regions(self):
        ...
        for e in event_table:
            ...
            event = SeafrogLocation(player, e["name"], None, stage)
...
class SeafrogItem(Item):
    game: str = "Seafrog"


class SeafrogLocation(Location):
    game: str = "Seafrog"
